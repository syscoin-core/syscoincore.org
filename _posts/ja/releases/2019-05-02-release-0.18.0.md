---
title: Syscoin Core 0.18.0
id: ja-release-0.18.0
name: release-0.18.0
permalink: /ja/releases/0.18.0/
excerpt: Syscoin Core バージョン0.18.0が利用可能になりました
type: releases
layout: page
lang: ja
date: 2018-05-02
...
optional_magnetlink: "magnet:?xt=urn:btih:a25c86ffa7a512b6d074287f74762b77f91cef4c&dn=syscoin-core-0.18.0&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969&tr=udp%3A%2F%2Fzer0day.ch%3A1337&tr=udp%3A%2F%2Fexplodie.org%3A6969"

# Note: it is recommended to check all links to ensure they use
#       absolute urls (https://github.com/syscoin/syscoin/doc/foo)
#       rather than relative urls (/syscoin/syscoin/doc/foo).

# Release notes from syscoin/syscoin 2472733a24a9364e4c6233ccd04166a26a68cc65 ("bump version to -final", 2019-04-30)
---
{% include download.html %}
{% githubify https://github.com/syscoin/syscoin %}
Syscoin Core バージョン0.18.0は以下から入手可能です。
...
`contrib/init/syscoind.service`にある`systemd`のUnit設定ファイルを使用している場合は、
`~syscoin/.syscoin`の代わりに`/var/lib/syscoind`をデータディレクトリとして使用するように
変更されました。新しい設定ファイルに切り替える際は、`/var/lib/syscoind`が存在するファイルシステムに
十分なスペースがあることを確認し（`df -h /var/lib/syscoind`で確認できます）、必要に応じて
...
- ZMQが追加のメッセージをドロップする前に、メモリ内のキューに入れるメッセージの最大数
  ...
  [ZMQのドキュメント](https://github.com/syscoin/syscoin/blob/master/doc/zmq.md#usage)を
  ご覧ください。
...
- `whitelistforcerelay`オプションはmempoolに受け入れられない場合でも、
  ...
  （将来この機能は廃止される可能性があるため、必要な場合はSyscoin Coreプロジェクトに
  [連絡](https://syscoincore.org/ja/contact/)し、ユースケースについてお知らせください）。

systemd initファイル {#systemd-init-file}
-----------------

systemdのinitファイル(`contrib/init/syscoind.service`)は、`~syscoin/.syscoin`
の代わりに`/var/lib/syscoind`をデータディレクトリとして使用するように変更されました。
この変更によりSyscoin Coreは他のサービスとの一貫性が高まり、systemdのinitの設定は
...
`contrib/init`以下で提供されるinitファイルを使用する際は、`/etc/syscoin/syscoin.conf`の
`datadir`オプションを上書きしても効果はありません。これはinitファイルで指定された
コマンドライン引数が`/etc/syscoin/syscoin.conf`で指定されたオプションよりも優先されるためです。

ドキュメンテーション {#documentation}
-------------

- JSON-RPCインターフェースに関する新しい[ドキュメント](https://github.com/syscoin/syscoin/blob/master/doc/JSON-RPC-interface.md)
  では、ウォレットの状態やmempoolの状態など、RPCの結果に異なるサブシステムの
  データソース間の不整合が含まれる可能性がある場合について説明しています。
  [RESTインターフェースのドキュメント](https://github.com/syscoin/syscoin/blob/master/doc/REST-interface.md)
  に同じルールが適用されることを示す注記が追加されています。

- このインターフェースを保護する方法に関する詳細情報が
  [JSON-RPCドキュメント](https://github.com/syscoin/syscoin/blob/master/doc/JSON-RPC-interface.md)
  に追加されています。
  
- `syscoin.conf`に関する新しい[ドキュメント](https://github.com/syscoin/syscoin/blob/master/doc/syscoin-conf.md)
  にSyscoin Coreを設定するための、その使用方法が記載されています。

- Syscoin CoreのBIP174 [Partially-Signed Syscoin Transactions
...
- [output script descriptor](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md)
...
- 新しい`--disable-bip70`オプションを`./configure`に渡すと、Syscoin-Qtへの
  BIP70のペイメントプロトコルのサポートやlibsslへのリンクを防止するすることができます。
  ペイメントプロトコルが過去Syscoin Coreにlibsslの脆弱性を露呈させたため、
  BIP70のサポートを必要としないユーザーは、将来の脆弱性にさらされる可能性を
  減らすためにこのオプションを使用することをお勧めします。

- （GUIビルド時の）Qtの最小バージョンが5.2から5.5.1になりました
  （[依存システム](https://github.com/syscoin/syscoin/blob/master/depends/README.md)
  は5.9.7を提供します）。
...
- `deriveaddresses`は[output descriptor](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md)
  に対応する１つ以上のアドレスを返します。
...
- `getaddressinfo`は、Syscoin CoreがアドレスのscriptPubKeyやオプションで
...
- `importprivkey`はインポートされる秘密鍵に対応したアドレスもしくは
  ...
  Syscoin Coreで"cold wallet"というラベルが付いたwatch-onlyアドレスを
...
- `importmulti`はdescriptorからのアドレスのインポートをサポートするようになりました。
  ...
  [こちら](https://github.com/syscoin/syscoin/blob/master/doc/descriptors.md)
  をご覧ください。
...
- 'account' APIはv0.17で廃止された後、削除されました。アカウントの代わりに
  ...
  詳細については[v0.17のリリースノート](https://github.com/syscoin/syscoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
  をご覧ください。
...
- 新しく`syscoin-wallet`ツールがSyscoin Coreの他の実行ファイルと一緒に
...
このセクションでは、他のSyscoinソフトウェアやサービスに影響を与える
可能性があるSyscoin Coreへの計画的な変更について説明します。

- バージョン0.16.0以降、Syscoin Coreの組み込みウォレットはユーザーが
  支払いを受けたい時にデフォルトでP2SHでラップしたsegwitアドレスを生成します。
  これらのアドレスは広く利用されている全てのソフトウェアと後方互換性が
  あります。（0.18から1年後に予定されている）Syscoin Core 0.20からは、
  Syscoin Coreはデフォルトでネイティブのsegwitアドレス（bech32）を
  使用するようになり、手数料の節約やその他の利点がもたらされます。
  現在、多くのウォレットやサービスが既にbech32アドレスへの送金を
  サポートしており、Syscoin Coreプロジェクトで十分な追加の採択が
  みられる場合は代わりにSyscoin Core 0.19.0で、デフォルトの受信アドレスが
  ...
  Syscoin Coreの0.16.0以上で`addresstype=bech32`設定オプションを
  設定しているかもしれません。）
...
- このリリースでは未使用の着信接続のスロットがある場合、（無効なデータを送信するなどの）
  ...
  以前は、Syscoin Coreは不正動作したピアのIPアドレスを一定期間（デフォルトで1日）
...
- Syscoin Coreのサブプロジェクトでは、コマンドラインユーザーがSyscoin Coreで
  いくつかの一般的なハードウェア鍵管理デバイスを使用できるようにする
  Hardware Wallet Interaction (HWI)スクリプトを提供します。
  詳しくは[プロジェクトページ](https://github.com/syscoin-core/HWI#readme)をご覧ください。

セキュリティ {#security}
--------

- このリリースでは、使用される乱数生成器（RNG）がOpenSSLのものからSyscoin Core
  独自の実装に変更されていますが、Syscoin Coreによって収集されたエントロピーは
