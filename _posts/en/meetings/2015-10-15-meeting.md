...
- [link to this week logs](http://syscoinstats.com/irc/syscoin-dev/logs/2015/10/15#l1444935660.0)  
- [Meeting minutes by meetbot](http://www.erisian.com.au/meetbot/syscoin-dev/2015/syscoin-dev.2015-10-15-19.01.html)
...
So far the devs are going with TheBlueMatt's proposal of [throwing away the cheapest txn and setting the min relay fee to it](https://github.com/syscoin/syscoin/pull/6722)
...
The average time in sipa's tests is 4ms. (After the meeting Morcos did some [benchmarking](https://github.com/syscoin/syscoin/pull/6722#issuecomment-148874040) and confirmed it was not specific to this PR, and pointed out the outliers come from CheckInputs and HaveInputs (as you might guess, having to do with checking the inputs)  
...
Review PR 6722 [Limit mempool by throwing away the cheapest txn and setting min relay fee to it](https://github.com/syscoin/syscoin/pull/6722)  
Morcos/sipa will do some more benchmarks and comment on the PR ( [morcos' benchmark results](https://github.com/syscoin/syscoin/pull/6722#issuecomment-148874040) )

## sendheaders BIP

### background  

[send headers BIP](https://github.com/syscoin/bips/blob/master/bip-0130.mediawiki)  
...
1. Extend the [version message](https://en.syscoin.it/wiki/Protocol_documentation#version).  
...
review [versionbits implementation](https://github.com/syscoin/syscoin/pull/6816)

## dev/discuss list policy

### background

The syscoin-dev mailing list is intended for technical discussions only. There's things that don't belong there but need to be discussed anyway.  
Now this is done in syscoin-dev, but the volume of this is getting too big.  
There's recently also an influx of really inappropriate posts, level [kindergarden](https://www.mail-archive.com/syscoin-dev@lists.linuxfoundation.org/msg02539.html).  
For the things that don't belong on syscoin-dev, but need to be discussed anyway there's a new list being created namely syscoin-discuss as well as clear policies and moderation for both.

### meeting comments

Syscoin-discuss was created, but the admin password wasn't distributed to jgarzik who's willing to guide the moderation.  
...
CheckLockTimeVerify (CLTV) repurposes the nSequence field (nSequence are 4 bytes intended for sequencing time-locked transactions, but this never got used). However, there's no way use these values in a syscoin script.   
CheckSequenceVerify (CSV) makes this field accessible to syscoin scripts.  

EDIT: [Turns out this is not entirely correct](https://www.reddit.com/r/Syscoin/comments/3pcinz/syscoin_dev_irc_meeting_in_laymans_terms_20151015/cw55n6q) as it is [relative locktime](https://github.com/syscoin/bips/blob/master/bip-0068.mediawiki) that repurposes the nSequence field. 
...
sipa points out a possible [bug](https://github.com/syscoin/syscoin/pull/6312#discussion_r41899674) that influences the wallet.   
CSV is not on target for the end of of the month, although a lot of work and progress has been made.  

### meeting conclusion

 - Review and ACK/NACK of 6312 [BIP-68: Mempool-only sequence number constraint verification](https://github.com/syscoin/syscoin/pull/6312)   
 - Review and ACK/NACK of 6566 [BIP-113: Mempool-only median time-past as endpoint for lock-time calculations](https://github.com/syscoin/syscoin/pull/6566)  
...
This summary was originally compiled by Stefan Gilis aka "G1lius" and posted to the [syscoin-dev mailing list][meetingsource] with the disclaimer, "Please bear in mind I'm not a developer so some things might be incorrect or plain wrong." and placed copyright in the Public Domain.

[meetingsource]: http://lists.linuxfoundation.org/pipermail/syscoin-dev/2015-October/011562.html
