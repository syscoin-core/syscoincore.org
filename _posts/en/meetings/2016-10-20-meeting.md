...
- [Link to this week logs](https://botbot.me/freenode/syscoin-core-dev/2016-10-20/?msg=75168477&page=6)
- [Meeting minutes by meetbot](http://www.erisian.com.au/meetbot/syscoin-core-dev/2016/syscoin-core-dev.2016-10-20-19.00.html)
 
---
 
## Notes / short topics

- Syscoin Core 0.13.1 [Release candidate 2](https://syscoin.org/bin/syscoin-core-0.13.1/test.rc2/) is out, currently without any reported issues.
...
Ideally consensus layer should decoupled from other parts of the Syscoin software. The long term goal is to extract a separate "libconsensus" library.
...
Recently Jorge Timón made a [mailinglist post](https://lists.linuxfoundation.org/pipermail/syscoin-dev/2016-October/013204.html) proposing a more detailed plan on how to proceed. He has his own [branch](https://github.com/jtimon/syscoin/commits/0.13-consensus-flags) where he's working on all the changes.
...
Jtimon would like to expose a "GetConsensusFlags" call in libconsensus, to hide [BIP9][] and previous development stuff similar to ["expose VerifyHeader"][#8493]. Sipa doesn't like turning the internal representation for headers into an interface and would just have an API where you can create a blockindexstore and you give it headers. That would mean libconsensus remains coupled with Syscoin Core's storage, which is sipa's preference. Wumpus notes the previous conclusion was that libconsensus should remain coupled with the current caching layer, but not with levelDB so that memory storage is part of libconsensus but not disk storage. Jtimon would be fine with having one with storage included and one without it, for example libsyscoin would likely never use a libconsensus that's coupled to syscoin's storage and concurrency. Others might just want to use core's current storage implementation to cut down on work. Sipa argues not abstracting out data structures leaves more opportunities for future optimizations.
...
[#8976]: https://github.com/syscoin/syscoin/pull/8976
