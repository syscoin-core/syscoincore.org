...
- rpc: deprecate addresses and reqSigs from rpc outputs [#20286](https://github.com/bitcoin/bitcoin/pull/20286).
  The following RPC calls are affected: `gettxout`, `getrawtransaction`, `decoderawtransaction`,
  `decodescript`, `gettransaction`, and REST endpoints: `/rest/tx`, `/rest/getutxos`, `/rest/block`.
  Two fields, `addresses` and `reqSigis`,  are removed from the output of the above commands. And new sane field `address` is added.
